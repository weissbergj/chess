...
int evaluate_captures(char captured) {
    switch (tolower(captured)) {
        case 'q': return 1000;  // Queen value
        case 'r': return 700;  // Rook value
        case 'b': case 'n': return 500;  // Bishop and Knight value
        case 'p': return 200;  // Pawn value
        default: return 0;
    }
}
...
int move_sort(char board[8][8], int src_row, int src_col, int dest_row, int dest_col, int move_count) {
    ...
    if (move_count < 15) {
        switch (tolower(piece)) {
            ...
            case 'k': 
                ...
                break;
            case 'q': 
                score -= 6000;  // Penalize early queen movement
                break;
        }
    }
    ...
    // Captures
    if (captured != '.') {
        score += evaluate_captures(captured) * 10;  // Prioritize captures strongly
    }
    ...
    if (tolower(piece) != 'k') {
        if (dest_col >= 3 && dest_col <= 4) {
            score += 10;  // Bonus for controlling central columns
        }
        if (tolower(piece) == 'n' && dest_row >= 2 && dest_row <= 5 && dest_col >= 2 && dest_col <= 5) {
            score += 15;  // Bonus for knights in central positions
        }
    }

...
int evaluate_position(char piece, int row, int col, int current_move_count, char board[8][8]) {
        ...
        for (int c = max(0, col - 2); c <= min(7, col + 2); c++) {
            ...
            if (is_white_piece(target_piece) != is_white_piece(piece) && target_piece != '.') {
                ...
                if (can_attack(target_piece, r, c, row, col, board)) {
                    int attacker_value = evaluate_captures(target_piece) * 2; // Value of attacker
                    capture_penalty -= attacker_value; // Scaled penalty
                }
            }
        }
    }
    capture_penalty = max(capture_penalty, -1000); // Increase the penalty cap
    ...
    // Check if the piece is in danger after the move
    if (is_white_piece(piece)) {
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                if (!is_white_piece(board[r][c]) && can_attack(board[r][c], r, c, row, col, board)) {
                    position_value -= 2000; // Apply a significant penalty for being attacked
                }
            }
        }
    }
    ...
    if (tolower(piece) == 'p') {
        ...
        for (int attack_col = col - 1; attack_col <= col + 1; attack_col += 2) {
            if (is_within_bounds(attack_row, attack_col) && board[attack_row][attack_col] != '.') {
                ...
                if (is_white_piece(target_piece) != is_white) {
                    capture_score += evaluate_captures(target_piece) * 2;
                }
            }
        }
        ...
        for (int d = 0; d < limit; d++) {
            ...
            while (is_within_bounds(attack_row, attack_col)) {
                ...
                if (target_piece != '.') {
                    if (is_white_piece(target_piece) != is_white) {
                        capture_score += evaluate_captures(target_piece) * 2; // Ensure rooks are valued
                    }

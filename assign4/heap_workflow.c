...
typedef struct {
    ...
    unsigned long mticks, fticks; // elapsed ticks malloc/free
...
static void run_workflow(int max_blocks, int max_size, int num_ops) {
    ...
    printf("Throughput:\n");
    printf("\t%ld malloc ops per ms \t(%d mallocs completed in %ld ticks)\n",
        (sim.mcount*1000*TICKS_PER_USEC)/sim.mticks, sim.mcount, sim.mticks);
    printf("\t%ld free ops per ms\t(%d frees completed in %ld ticks)\n",
        (sim.fcount*1000*TICKS_PER_USEC)/sim.fticks, sim.fcount, sim.fticks);
